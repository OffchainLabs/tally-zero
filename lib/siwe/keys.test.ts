...
describe("siweKeys", () => {
  ...
  // useActAs invalidates SAFES_SCOPE rather than reconstructing a per-signer
  // key, which only works because query keys match by prefix. If the Safe key
  // stopped nesting under the root, that invalidation would silently no-op and
  // a newly remembered Safe would never appear in the switcher.
