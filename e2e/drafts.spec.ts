...
test("draft lifecycle: save, reopen, publish, share, mark submitted", async ({
  ...
  const page = await signedInPage("drafts", "/proposal/new");
  ...
  // requireSession, so an anonymous reader gets the sign-in gate in place of
  // the form rather than a form that would 401.
  await expect(anonPage.getByTestId("mark-submitted")).toHaveCount(0);
  await expect(anonPage.getByTestId("siwe-connect")).toBeVisible();
  ...
  const submitter = await signedInPage("second", `/drafts/shared/${slug}`);
...
test("deletes an unpublished draft", async ({ signedInPage }) => {
  const page = await signedInPage("drafts", "/proposal/new");
...
test("refuses to save a draft with no description", async ({
  signedInPage,
}) => {
  const page = await signedInPage("drafts", "/proposal/new");
